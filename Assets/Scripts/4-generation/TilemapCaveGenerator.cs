...
public class TilemapCaveGenerator: MonoBehaviour {
    ...
    [Tooltip("To check how many places the player has to go")]
    [SerializeField] GameObject player;

    //[Tooltip("To check how many places the player has to go")]
    [SerializeField] int playerSpace;
    ...
    void Start()  {
        ...
        while (!succeeded)
        {
            ...
            if (!allowedTiles.Contain(tilemap.GetTile(startNode)))
            {
                continue;
            }
            int spaceSize = BFS.GetSpace(tilemapGraph, startNode, playerSpace).Count;
            Debug.Log("Count = " + spaceSize);
            succeeded = (spaceSize >= playerSpace);
        }
